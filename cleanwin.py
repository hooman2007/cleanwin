...
# QSS stylesheet for consistent UI styling
STYLESHEET = """
QMainWindow, QDialog {
...
# List of programs to remove with their metadata
PROGRAMS = [
...
class LanguageDialog(QDialog):
    ...
    def __init__(self):
        ...
        self.setWindowIcon(QIcon(os.path.join(ICONS_DIR, "default.png")))
...
class AboutDialog(QDialog):
    """Dialog displaying information about CleanWin."""
    def __init__(self, lang):
        ...
        title_icon = QLabel()
        title_icon.setPixmap(QPixmap(os.path.join(ICONS_DIR, "default.png")).scaled(48, 48, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        desc_icon = QLabel()
        desc_icon.setPixmap(QPixmap(os.path.join(ICONS_DIR, "about_icon.png")).scaled(40, 40, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        desc = QLabel(
        ...
        dev_icon = QLabel()
        dev_icon.setPixmap(QPixmap(os.path.join(ICONS_DIR, "developer_icon.png")).scaled(40, 40, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        website_icon = QLabel()
        website_icon.setPixmap(QPixmap(os.path.join(ICONS_DIR, "website_icon.png")).scaled(40, 40, Qt.KeepAspectRatio, Qt.SmoothTransformation))
        ...
        email_icon = QLabel()
        email_icon.setPixmap(QPixmap(os.path.join(ICONS_DIR, "email_icon.png")).scaled(40, 40, Qt.KeepAspectRatio, Qt.SmoothTransformation))
...
class MainWindow(QMainWindow):
    """Main application window for CleanWin."""
    def __init__(self, lang):
        super().__init__()
        self.lang = lang
    ...
    def populate_cards(self):
        """Populate the grid with app cards based on search filter."""
    ...
    def select_all(self):
        """Toggle selection of all app checkboxes."""
        state = not all(cb.isChecked() for cb in self.cards)
        for cb in self.cards:
            cb.setChecked(state)

    def remove_apps(self):
        """Remove selected apps using PowerShell."""
    ...
    def show_guide(self):
        ...
        dlg.setWindowIcon(QIcon(os.path.join(ICONS_DIR, "default.png")))
        if self.lang == "fa":
...
def main():
    """Entry point for the CleanWin application."""
